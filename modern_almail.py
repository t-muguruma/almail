...
class ModernALMail:
    ...
    def setup_ui(self):
        ...
        help_menu.add_command(label="バージョンアップ確認", command=lambda: messagebox.showinfo("確認", "最新バージョンを使用しています。"))
    ...
    def open_account_edit_dialog(self, account_id, callback):
        ...
        if account_id:
            cursor = self.conn.cursor()
            cursor.execute("SELECT email, smtp_server, smtp_port, imap_server, imap_port, username, password, display_html_as_text, minimize_to_tray, notify_new_mail, auto_receive_enabled, auto_receive_interval, signature FROM accounts WHERE id = ?", (account_id,))
            row = cursor.fetchone()
            if row:
        ...
        def save():
            ...
            params = (data['email'], data['smtp_server'], data['smtp_port'], data['imap_server'], data['imap_port'], data['username'], data['password'], 
                      int(html_as_text_var.get()), int(tray_var.get()), int(notify_var.get()), int(auto_rx_enabled_var.get()), int(interval_ent.get() or 10), sig_text.get("1.0", tk.END))
            
            if account_id:
                self.conn.execute("""UPDATE accounts SET email=?, smtp_server=?, smtp_port=?, imap_server=?, imap_port=?, username=?, password=?, 
                                     display_html_as_text=?, minimize_to_tray=?, notify_new_mail=?, auto_receive_enabled=?, auto_receive_interval=?, signature=? WHERE id=?""", params + (account_id,))
            else:
                self.conn.execute("""INSERT INTO accounts (email, smtp_server, smtp_port, imap_server, imap_port, username, password, 
                                     display_html_as_text, minimize_to_tray, notify_new_mail, auto_receive_enabled, auto_receive_interval, signature) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""", params)
...
"""
    ...
    def open_about(self):
        ...
        tk.Label(win, text="Version 1.0.0 (Multi-Account Ready)", font=("Arial", 10)).pack()
