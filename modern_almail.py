...
# tkinterweb 等がエラー出力を試みてクラッシュするのを防ぎます。
if getattr(sys, 'frozen', False) and sys.stdout is None:
    # 実行ファイルと同じディレクトリにログを出力します。
...
class ModernALMail:
    ...
    DOWNLOAD_BASE_URL = "https://raw.githubusercontent.com/t-muguruma/almail/master/"
    # ここまで t.muguruma

    def _parse_version_string(self, version_str):
        """バージョン文字列を (major, minor, patch) の数値タプルに変換"""
        try:
    ...
    def _record_current_version(self):
        """現在のプログラムバージョンをDBに記録（履歴になければ追加）"""
        major, minor, patch = self._parse_version_string(self.APP_VERSION)
        cursor = self.conn.cursor()
        cursor.execute("SELECT id FROM app_versions WHERE major=? AND minor=? AND patch=?", (major, minor, patch))
        if not cursor.fetchone():
            cursor.execute("INSERT INTO app_versions (major, minor, patch, content) VALUES (?, ?, ?, ?)", 
    ...
    def __init__(self, root):
        ...
        self.compose_win = None  # 作�E画面の管琁E��
        self.auto_receive_job = None # 自動受信タイマ�E用
        self.preview_images = [] # 画像�Eレビューの参�E保持用
        self.tray_icon = None # シスチE��トレイアイコン用
        self.current_message_account_id = None # 現在選択中のメールが属するアカウンチED
        self.tk_icon = None # ウィンドウアイコン用
        self.pil_icon = None # トレイアイコン用
        self._skip_auto_select_first = False # 受信時�E自動フォーカス制御用フラグ

        # 1. 起動画面�E�スプラチE��ュスクリーン�E�を表示
        self.root.withdraw()  # メインウィンドウを一旦隠ぁE
        splash = tk.Toplevel(self.root)
        splash.overrideredirect(True)  # 枠めE��イトルバ�Eを消す

        # ロゴ画面のチE��イン�E�白背景にブルーのアクセント！E
        w, h = 450, 250
    ...
    def _setup_app_icon(self):
        ...
        if not self.pil_icon:
            # トレイアイコンが読み込めなかった場合、メイン用をリサイズして代用
    ...
    def setup_ui(self):
        ...
        btn_delete = tk.Button(self.toolbar, text="🗑 削除", bg="#ffebee", command=self.delete_selected_mail, relief=tk.FLAT, padx=10)
        ...
        self.btn_reply = tk.Button(self.action_bar, text="↩ 返信", bg="#ffffff", command=self.reply_mail, relief=tk.FLAT, padx=10, font=("MS UI Gothic", 9))
        self.btn_reply.pack(side=tk.LEFT, padx=2, pady=2)
        self.btn_forward = tk.Button(self.action_bar, text="↪ 転送", bg="#ffffff", command=self.forward_mail, relief=tk.FLAT, padx=10, font=("MS UI Gothic", 9))
        self.btn_forward.pack(side=tk.LEFT, padx=2, pady=2)
        self.btn_pane_delete = tk.Button(self.action_bar, text="🗑 削除", bg="#ffffff", fg="#c62828", command=self.delete_selected_mail, relief=tk.FLAT, padx=10, font=("MS UI Gothic", 9))
    ...
    def check_message_security(self, msg_id):
        ...
        if not auth_res or not auth_res.strip():
            self.auth_banner.config(text="🛡 認証情報なし(古いメール、またはサーバー未対応)", bg="#9e9e9e", fg="white")
            return
        ...
        if is_suspicious:
            ...
            self.auth_banner.config(text=f"⚠ 認証不完全: {status_text}", bg="#ffc107", fg="#333333")
    ...
    def update_home_view(self):
        ...
        if recent:
            for r_id, subj, snd, dt, fld, acc_id in recent:
                ...
                # フォルダの内部ID (FLD_{acc_id}_{folder}) を生成してジャンプ用に持たせる
    ...
    def open_compose_window(self, to="", subject="", body="", from_account_id=None, draft_id=None):
        """新規メール作成ウィンドウを開く"""
        # 既に作成画面が開かれている場合は、そのウィンドウを前面に出す
        ...
        if to: body_txt.mark_set("insert", "1.0") # 返信時の先頭にカーソル
    ...
    def open_address_book(self):
        ...
        ttk.Label(input_frame, text="ニックネーム").grid(row=0, column=0, padx=5, pady=5)
    ...
    def open_settings(self):
        ...
        # --- タブ: 全般・詳細設定 ---
        # マルチアカウント化に伴う利便性のために共通的な設定をここに表示します。
        ...
        ttk.Button(container, text="設定を全てのアカウントに適用して保存", command=save_common_settings).pack(pady=20, anchor="w")
    ...
    def run_import(self, filter_tables):
        ...
        try:
            ...
            for table, rows in data.items():
                ...
                if table == 'accounts':
                    cursor.execute("DELETE FROM accounts") # アカウントの上書き
    ...
    def refresh_folders(self):
        ...
        for acc_id, email_addr in cursor.fetchall():
            ...
            self.folder_tree.insert(acc_node, "end", iid=f"FLD_{acc_id}_Trash", text="  🗑 ゴミ箱")
    ...
    def on_folder_select(self, event):
        ...
        # フォルダ選択後、メッセージはまだ選択されていない場合
        if not children:
            self.auth_banner.config(text="🛡 メールがありません", bg="#9e9e9e", fg="white")
    ...
    def on_message_select(self, event):
        ...
        try:
            ...
            if folder and folder.lower() == "sent":
                ...
                self.auth_banner.pack(side=tk.TOP, fill=tk.X) # 非送信済みメールの場合に再表示
    ...
    def _handle_google_security_alert(self, body, body_html, attachments_full):
        ...
        <div class="petal-card">
            <div class="petal-header">🛡 Google アカウントの保護</div>
            <div class="petal-row">
    ...
    def open_manual(self):
        ...
        content = """【Petal マニュアル、E

■ アプリの概要E
Petalは、AL-Mailの使ぁE��手を踏襲しつつ、現代皁E��インターフェースと機�Eを備えたマルチアカウント対応�Eメールクライアントです、E

■ 主な機�E
1. マルチアカウント管琁E
   設定画面から褁E��のメールアカウントを追加・管琁E��きます、E
2. 統合受信箱
   すべてのアカウント�E受信メールを一覧で確認できます、E
3. 添付ファイル管琁E
   メール冁E�E画像を直接プレビューしたり、すべての添付ファイルを一括管琁E��たりできます、E
4. AL-Mailインポ�EチE
   既存�EAL-MailのメールボックスからチE�Eタを簡単に移行できます、E
5. セキュリチE��チェチE��
   SPF/DKIM/DMARCなどの認証結果を�EかりめE��く表示し、なりすましを警告します、E

■ 基本皁E��使ぁE��
・メール受信: チE�Eルバ�Eの「受信」�Eタン、また�Eメニューの「ファイル」�E「メール受信」を選択します、E
・新規作�E: チE�Eルバ�Eの「新規」�Eタンを選択します。差出人アカウント�Eコンボ�EチE��スで刁E��替え可能です、E
・返信/転送E メール詳細画面の上部にあるボタンを使用します、E
・削除: 不要なメールを選択して「削除」�Eタンを押すと、ゴミ箱へ移動します、E
・アドレス帳: 頻繁に送信する相手を登録して、新規作�E時に呼び出すことができます、E
"""
